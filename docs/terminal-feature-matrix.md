...
| 鼠标历史浏览 | 每个滚轮事件移动一条正文消息，跳过进场与点赞；从回复选择切换到滚轮时沿用当前消息位置并取消回复选择；新消息到达保持阅读锚点。高对比状态条持续显示历史模式、Esc 返回提示、新消息数量与可选倒计时；Esc、End 或向下滚回底部恢复实时跟随；Esc 不退出 TUI |
| 历史空闲返回 | 默认仅手动；`/history [秒数\|off]` 设置本次会话，TOML `history_idle_seconds` 持久设置，CLI `--history-idle-seconds` 优先覆盖，0/off 关闭自动返回。启用后手动返回仍可用；按键及滚轮等操作重置计时，消息到达不重置；自动返回保留草稿；二维码、密码或停播弹窗期间暂停计时 |
...
| 实时状态分层 | 顶部固定两行：第一行左侧为 LIVE/OFFLINE/ROTATING 操作状态，直播间标题使用主题 `rank` 语义色绝对居中，右侧为已开播时长；第二行左侧显示主播名称，右侧整体右对齐显示 OBS 与 MIC 状态；OBS 使用单个 `●`，已连接为绿色、未连接为红色。OMP Box 上边框左侧发送状态使用主题 `success`、`info`、`warning` 语义色；右侧仅显示 ◉、♥、▤、● 及数值。主播弹幕名称前的 `♚` 使用 `rank`，主播名使用 `host`。LIVE 红色圆点与文字每 500 ms 同步显隐，移除红底、保持占位宽度不变；右侧计时为白字深色底，不再使用蓝色底块，位置和计时逻辑不变。不依赖 ANSI Blink 或 RGB 色差，离线与轮播状态不闪烁。 |
...
| OBS 控制 | `/obs`、`/obs status|connect|mute|unmute|scene|start|stop`；每次异步操作立即显示进度，认证、密码和连接失败给出可执行提示；场景和输入候选读取 OBS 真相；`/obs stop` 弹窗默认选中“返回”，方向键选“确认”并按 Enter 后倒计时 3 秒停播，Esc 可在请求发出前取消；停播不退出 TUI |
